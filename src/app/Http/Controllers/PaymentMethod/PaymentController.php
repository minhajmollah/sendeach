...
class PaymentController extends Controller
{
    ...
    public function preview()
    {
        ...
        $subscription = session('subscription_id') ?
            Subscription::where('id' , session('subscription_id'))->where('user_id' , $userId)->whereIn('status' , [0 , 1 , 2])->first() : null;
    ...
    public static function paymentUpdate($trx)
    {
        ...
        if ($paymentData && $paymentData->status == 0) {
            ...
            if ($paymentData->subscriptions_id) {
                $subscription = Subscription::where('id' , $paymentData->subscriptions_id)->first();
                $subscription->status = 1;
                $subscription->save();

                $credits = $subscription->plan->credit;
                $emailCredits = $subscription->plan->email_credit;
                $whatsappCredits = $subscription->plan->whatsapp_credit;
            } else {
                $credits = ceil(CreditLog::getCreditsPerDollar() * $paymentData->amount);
                $emailCredits = 0;
                $whatsappCredits = 0;
            }
            ...
            $creditInfo->details = $subscription ? $subscription->plan->name . " Plan Purchased" : 'Bought Credits';
            $creditInfo->save();
            ...
            $emailCredit->details = $subscription ? $subscription->plan->name . " Plan Purchased" : 'Bought Credits';
            $emailCredit->save();
            ...
            $whatsappCredit->details = $subscription ? $subscription->plan->name . " Plan Purchased" : 'Bought Credits';
            $whatsappCredit->save();
