            ...
            <div class="row gap-2">
                <div class="row justify-content-between align-items-center gap-2 m-0 mb-2 p-0">
                    <div class="d-flex col-lg-4 gap-2 col-sm-12 col-md-6 justify-content-start">
                        <div class="">
                            <button class="btn btn--danger delete_campaign"
                                ...
                                @forelse($reports as $report)
                                    <tr class="@if($loop->even) table-light @endif">
                                        ...
                                        <td data-label="{{ translate('Batch Started At')}}">
                                            {{\Carbon\Carbon::parse($report->started_at)->toFormattedDateString()}}
                                        </td>

                                        <td data-label="{{ translate('Batch Completed At')}}">
                                            {{ !($report->pending || $report->processing)
                                            ? \Carbon\Carbon::parse($report->completed_at)->toFormattedDateString() : '-'}}
                                        </td>

                                        <td data-label="{{ translate('Status')}}">
                                            {{$report->delivered}} Delivered Out Of {{ $report->total }}
                                        </td>

                                        <td data-label={{ translate('Action')}}>
                                            <a class="btn--primary text--light details campaign-view-btn"
                                               data-message="{{$report->message}}"
                                               data-batch-id="{{$report->batch_id}}"
                                               data-total="{{$report->total}}"
                                               data-pending="{{$report->pending}}"
                                               data-processing="{{$report->processing}}"
                                               data-delivered="{{$report->delivered}}"
                                               data-failed="{{$report->failed}}"
                                               data-link="{{route($routeStr.'pending', ['batch_id' => $report->batch_id])}}"
                ...
                <form action="{{ $deleteRoute }}"
                    ...
                    <div class="modal_body2">
                        ...
                        <div class="modal_text2 mt-3">
                            ...
                            <p class="text-muted small">If Nothing is selected, the logs with no batch id will be deleted.</p>
                        </div>
                    </div>
