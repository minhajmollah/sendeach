		                    ...
		                    @forelse($transactions as $transaction)
			                    <tr class="@if($loop->even) table-light @endif">
	                            	...
	                            	<td data-label="{{ translate('User')}}">
			                    		<a href="{{route('admin.user.details', $transaction->user_id)}}" class="fw-bold text-dark">{{__($transaction->user->email)}}</a>
				                    </td>
				                    ...
				                    <td data-label="{{ translate('Amount')}}">
				                    	<span class="@if($transaction->transaction_type == "+") text--success @else text--danger @endif">
				                    		{{$transaction->transaction_type == "+" ? '+' : '-'}}{{shortAmount($transaction->amount)}} {{__($general->currency_name)}}</span>
				                    </td>
